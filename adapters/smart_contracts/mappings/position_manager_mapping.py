...
def to_position_manager_model(name: str, address: str, position_worth: int, cost_basis: int, pnl: int, exposures: list, allocation: list, price: int) -> PositionManager:
	exposures = to_exposure_models(exposures)
	allocations = to_allocation_models(allocation)

	return PositionManager(name, address, position_worth, cost_basis, pnl, exposures, allocations, price)

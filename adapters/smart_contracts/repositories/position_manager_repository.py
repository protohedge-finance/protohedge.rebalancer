...
position_manager_abi = json.load(
    open("adapters/smart_contracts/abi/position_manager.json"))
...
class PositionManagerRepository:
    ...
    def get_position_manager(self, address: str) -> PositionManager:
        position_manager_contract = self.w3.eth.contract(
            address=address, abi=position_manager_abi)
